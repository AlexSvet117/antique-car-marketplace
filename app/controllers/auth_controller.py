...
def login():
    ...
    if not user.check_password(data.get("password")):
        return jsonify({"error" : "The password provided doesn't match the original"})
    
    access_token = create_access_token(identity=str(user.id), additional_claims={"email": data.get("emails")})
